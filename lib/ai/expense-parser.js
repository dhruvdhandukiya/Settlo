...
1. WHO PAID (PAYER DETECTION):
   - If text mentions someone else paid (e.g. "Alex has paid bill for me", "Alex paid $50", "Sam paid for dinner", "Alex ne diya", "Alex ne pay kiya", "Alex ne bhara"):
     * Set "paidByUserId" to that person's userId (e.g., Alex's userId).
     * In "splits", include both the payer and the person(s) who were covered or shared the expense.
     * Set "paid: true" for the payer, and "paid: false" for all other participants who owe money.
   - If text says "I paid", "paid by me", "maine pay kiya", "maine diya", "mere card se", or no other payer is named, "paidByUserId" is "${currentUser.id}".
...
3. MULTILINGUAL & HINGLISH / CODE-MIXED SUPPORT:
   - Understand Hindi / Hinglish / Romanized code-mixed phrases seamlessly:
     * Split phrasing: "aadha aadha", "half-half", "50-50", "dono mein baant do", "divide kar de", "barabar split kar", "sab mein baant do" -> splitType: "equal".
     * Exact phrasing: "Alex ka 200 baaki mera" -> Alex owes 200, current user owes remaining.
     * Amount & units: "₹450", "450 rs", "450 rupaye", "450 bucks", "1.5k" (= 1500), "2k" (= 2000), "450 gaya", "bill bana 450".
     * Category inference:
       - "chai", "sutta", "biryani", "dhaba", "zomato", "swiggy", "ccd", "restaurant", "drinks" -> "foodDrink"
       - "auto", "cab", "ola", "uber", "petrol", "diesel", "metro" -> "transportation"
       - "kirana", "ration", "blinkit", "zepto", "instamart", "groceries" -> "groceries"
       - "hotel", "room", "goa trip", "manali", "flight", "train", "tickets" -> "travel"
       - "wifi", "bijli bill", "electricity", "rent", "recharge" -> "utilities"
4. DEFAULT GROUP BEHAVIOR: If an expense involves a contact and no group is specified (e.g. "Dinner with Alice $50"), treat it as a 1-on-1 personal expense (groupId: null).
5. RECEIPT PHOTO HANDLING:
   - If an image/photo of a receipt is provided, return Type 2 ("receipt_analysis") extracting merchant, line items, total, tax, tip.
   - If user provided notes (e.g. "Alex paid for Sam"), also identify "suggestedPayerUserId" and "suggestedParticipantUserIds".
6. AMOUNT EXTRACTION:
   - Always extract the numeric value from text like "$45", "450 rs", "₹500", "45 dollars", "spent 45", "1.5k", etc., and set "amount" as a valid float number.
...
7. DATE HANDLING:
   - Use current timestamp (${Date.now()}) unless a specific relative date ("yesterday", "kal", "last Friday") or explicit date is mentioned in the text.
8. AMBIGUITY: Only return Type 3 ("clarification_needed") if participants are completely missing or no amount can be found anywhere in the text or receipt.
9. ARITHMETIC: In Type 1 ("expense_proposal"), splits must sum up exactly to the total amount.
10. CURRENCY EXTRACTION:
   - Identify currency symbols (₹, $, €, £, ¥, د.إ, C$, A$, etc.), ISO codes (INR, USD, EUR, GBP, AED, CAD, JPY, etc.), or geographical/tax clues:
     * Indian Rupee (₹, Rs, INR, rupaye, GST, CGST, SGST, Chennai, Mumbai, Delhi, Bengaluru, Tamil Nadu, etc.) -> "INR"
     * US Dollar ($, USD, bucks, dollars) -> "USD"
     * Euro (€, EUR, euros) -> "EUR"
     * British Pound (£, GBP, pounds) -> "GBP"
     * UAE Dirham (AED, د.إ, dirhams, Dubai, UAE) -> "AED"
     * Canadian Dollar (CAD, C$) -> "CAD"
     * Japanese Yen (JPY, ¥, yen) -> "JPY"
   - Always return "currency" as a 3-letter ISO code in uppercase (e.g. "INR", "USD", "EUR", "GBP", "AED").
   - If no currency is visible, default to "${currentUser.currency || "INR"}".

JSON RESPONSE SCHEMAS & EXAMPLES:

Type 1 - Expense Proposal (English or Hinglish):
Example 1: "Bhai CCD pe 450 bill aaya, maine pay kiya Alex aur mere beech aadha aadha split kar"
{
  ...
  "splitType": "equal",
  "splits": [
    { "userId": "${currentUser.id}", "amount": 225.0, "paid": true },
    { "userId": "user_id_of_alex", "amount": 225.0, "paid": false }
  ],
  "reasoning": "CCD bill of 450 paid by user, split equally between user and Alex (225 each)",
  "confidence": 0.95
}

Example 2: "Uber gaadi mein 300 gaya, Alex ne pay kiya mera aur uska 50-50"
{
  "type": "expense_proposal",
  "description": "Uber",
  "amount": 300.0,
  ...
  "date": ${Date.now()},
  "paidByUserId": "user_id_of_alex",
  "groupId": null,
  "splitType": "equal",
  "splits": [
    { "userId": "user_id_of_alex", "amount": 150.0, "paid": true },
    { "userId": "${currentUser.id}", "amount": 150.0, "paid": false }
  ],
  "reasoning": "Uber ride 300 paid by Alex, split 50-50 with user (150 each)",
  "confidence": 0.95
}
...
export function validateAndSanitizeResponse({
  ...
  if (parsed.type === "expense_proposal" || parsed.amount !== undefined) {
      ...
      sanitizedSplits.push({
        ...
        amount:
          typeof split.amount === "number"
            ? Math.round(Math.abs(split.amount) * 100) / 100
            : 0,
        paid: Boolean(split.paid),
      });
    }

    // Ensure payer has a split entry
    if (!sanitizedSplits.some((s) => s.userId === validatedPaidBy)) {
      sanitizedSplits.push({
        userId: validatedPaidBy,
        amount: 0,
        paid: true,
      });
    }

    // Fallback: If no participants were recognized, add currentUser
    ...
    const proposalCurrency = parsed.currency
      ? String(parsed.currency).toUpperCase().trim()
      : (currentUser.currency || "INR");
